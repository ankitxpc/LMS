...
namespace LMS
{
    public partial class UserForm : Form
    {
        ...
        private void btnSaveAndNew_Click(object sender, EventArgs e)
        {
            ...
            //if (!Regex.Match(TxtPassword.Text, "@" [\s] + $"").Success)
            //{
            //    MessageBox.Show("Allowed no whiteSpaces!!", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    TxtPassword.Clear();
            //    TxtPassword.Focus();
            //    return;
            //}
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (ddlSearch.SelectedIndex == 1)
            {
                ...
                {
                    SqlDataAdapter da = new SqlDataAdapter("select * from users where userid like '" + txtSearch.Text + "%' ", con);
                    da.Fill(dt);
                    grdUser.DataSource = dt;
                }
                else
